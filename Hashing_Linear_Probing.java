...
public class Hashing_Linear_Probing
{
    ...
    public static void insert (String[] array)
    {
        for (int i=0;i<array.length;i++)
        {
            ...
            collisions =0;
            numKeys++;
            int LPIndex = linearProbing(hashArray,initialIndex,collisions);
            ...
            else
            {
                ...
                hashArray[LPIndex] = array[i];
            }
            System.out.println(array[i]+"   "+ "Initial Index: "+initialIndex+"   "+"Linear Probing Index:"+LPIndex
                    + "   Collisions: "+collisions);
        }
    }

    public static int linearProbing(String[] hashArray, int index, int collision)
    {
        ...
        if(hashArray[index] == null)
        {
            ...
            return index; // insert possible at this locatiom
        }
        collisions = ++collision;
        return linearProbing(hashArray,++index,collision); // recursive call
    }
    ...
    {
        int index = -1; //initial
        int hashID = hashFunction(word,hashArray.length);
        while (hashArray[hashID] != null )
        {
            if(hashArray[hashID].equals(word))
            {
                ...
                hashID = (hashID + 1) % hashArray.length;
